...
import blurShaderCode from "./shaders/blurShader.wgsl?raw";
...
function updateCanvas(renderContext) {
  ...
  canvas.gridWidth = Math.floor(
    canvas.width / renderContext.config.core.grid.cellSize
  );
  canvas.gridHeight = Math.floor(
    canvas.height / renderContext.config.core.grid.cellSize
  );
...
async function createBlurPipeline(renderContext) {
  const device = renderContext.gpu.device;
  ...
  const blurPipeline = device.createRenderPipeline({
    ...
    fragment: {
      module: blurShaderModule,
      entryPoint: "fragmentMain",
      targets: [{ format: renderContext.canvas.canvasFormat }],
    },
    primitive: {
  ...
  return { blurPipeline, blurBindGroupLayout };
}
...
function createStorageBuffer(renderContext, stateArray, bufferLabel) {
...
function createBlurBindGroup(renderContext, renderTarget, blurBindGroupLayout) {
  const device = renderContext.gpu.device;
  ...
  return blurBindGroup;
}

function createRenderTarget(renderContext) {
  const renderTexture = renderContext.gpu.device.createTexture({
    size: {
  ...
  const renderTarget = renderTexture.createView();
  return renderTarget;
}
...
export async function Init(config) {
  ...
  rend.blurBindGroupLayout = blurBindGroupLayout;
  rend.postBindGroup = createBlurBindGroup(
    rend,
    rend.renderTarget,
...
export function Render(renderContext, numInstances, bindGroup) {
  ...
  const workgroupHeight = Math.ceil(
    renderContext.canvas.gridHeight / workgroupSize
  );
...
function updateState(renderContext) {
  ...
  renderContext.gpu.device.queue.writeBuffer(
    buffer,
    0,
    renderContext.drawArray
  );
}
...
export function Tick(renderContext, deltaTime) {
  ...
  Render(
    renderContext,
    renderContext.canvas.gridWidth * renderContext.canvas.gridHeight,
    renderContext.bindGroups[renderContext.renderStep % 2]
  );
}
